...
export function StatsApp() {
    ...
    const [dataset1Index, setDataset1Index] = useState<number>(0); // Sets the first dimension data
    const [dataset2Index, setDataset2Index] = useState<number>(2); // Sets the second dimension data
                    ...
                    <div style={{ margin: '100px 0 20px 0' }}>
                        ...
                        <Row className='row mb-1'>
                            <ButtonGroup className='btn-group' style={{ outline: '1px solid black' }}>
                                <div className={`btn`}>1st Dimension: (temporary nav)</div>
                                {
                        ...
                        <Row className='row mb-1'>
                            <ButtonGroup className='btn-group' style={{ outline: '1px solid black' }}>
                                <div className={`btn`}>2nd Dimension: (temporary nav)</div>
                                {
                        ...
                        <div className='' style={{ outline: '1px solid black' }}>
                            <div>1</div>
                            {
                                FACET_STORE.map((facet, index) => {
                                    ...
                                    return <div key={index}><a href={url} key={index}>{url}</a></div>
                                })
                            }
                            <div>2</div>
                            {
                                FACET_STORE.map((f1, index) => {
                                    ...
                                    return ADDITIONAL_FACET_STORE.filter((f2) => f2.facetName !== 'None' && f2.facetName !== f1.facetName).map((f2, index) => {
                                        return <div key={index}><a href={url} key={index}>{url}&facet2={f2.facetName}</a></div>
                                    })
                                })
                            }
                        </div>
...
function StatsNavBar() {
    ...
    return <div>
        ...
        <StatsAppModal show={showModal} handleClose={() => { setShowModal(false) }} title={`Search API`} footer={`Footer Placeholder`}>
